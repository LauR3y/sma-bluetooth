...
void SbContext::send(const std::vector<uint8_t>& message) {
...
std::vector<uint8_t> SbContext::buildMessage61() {
...
std::vector<uint8_t> SbContect::append(std::vector<uint8_t>& message, const std::vector<uint8_t>& toAppend) {
	message.insert(message.end(), toAppend.begin(), toAppend.end());
    return message;
}
