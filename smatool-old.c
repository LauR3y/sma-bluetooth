 ...
 */
u16
pppfcs16 (u16 fcs, void *_cp, int len)
{
...
int
check_send_error (ConfType * conf, int *s, int *rr, unsigned char *received,
		  ...
		  int *already_read)
{
  ...
  if (FD_ISSET ((*s), &readfds))
    {				// did we receive anything within 5 seconds
  ...
  if (bytes_read > 0)
	...
	(*terminated) = 0;
      for (i = 0; i < bytes_read; i++)
	{			//start copy the rec buffer in to received
	  if (buf[i] == 0x7d)
	    {			//did we receive the escape char
	      switch (buf[i + 1])
		{		// act depending on the char after the escape char
...
int
read_bluetooth (ConfType * conf, int *s, int *rr, unsigned char *received,
		int cc, unsigned char *last_sent, int *terminated)
{
  int bytes_read, i, j;
  unsigned char buf[1024];	/*read buffer */
...
unsigned char *
get_timezone_in_seconds (unsigned char *tzhex)
{
  time_t curtime;
  struct tm *loctime;
  struct tm *utctime;
  ...
  if (debug == 1)
    printf
      ("utc=%04d-%02d-%02d %02d:%02d local=%04d-%02d-%02d %02d:%02d diff %d hours\n",
       ...
       utctime->tm_hour, utctime->tm_min, year, month, day, hour, minute,
...
char *
sunrise (float latitude, float longitude)
{
  ...
  L =
    M + (1.916 * sin ((pi / 180) * M)) + (0.020 * sin (2 * (pi / 180) * M)) +
    282.634;
  if (L > 360)
    L = L - 360;
  if (L < 0)
  ...
  cosH =
    (cos ((pi / 180) * zenith) -
     (sinDec * sin ((pi / 180) * latitude))) / (cosDec * cos ((pi / 180) *
							      latitude));
  ...
  if (localT > 24)
    localT = localT - 24;
  sprintf (returntime, "%02.0f:%02.0f", floor (localT),
	   floor ((localT - floor (localT)) * 60));
  return returntime;
}

char *
sunset (float latitude, float longitude)
{
  //adapted from http://williams.best.vwh.net/sunrise_sunset_algorithm.htm
  time_t curtime;
  struct tm *loctime;
...
long
ConvertStreamtoLong (unsigned char *stream, int length,
...
//Convert a recieved string to a value
float
ConvertStreamtoFloat (unsigned char *stream, int length, float *value)
{
  int i, nullvalue;
	      ...
	      if (data_follows == 1)
		{
		  ...
		  if (sscanf (line, "%x %x", &tmp.key1, &tmp.key2) == 2)
		      ...
		      for (i = 6; line[i] != '\0'; i++)
			{
		      ...
		      if (sscanf
			  (line + i + 1, "%s %f", tmp.units,
			   &tmp.divisor) == 2)
			{

			  if ((*num_return_keys) == 0)
			    returnkeylist =
			      (ReturnType *) malloc (sizeof (ReturnType));
			  else
			    returnkeylist =
			      (ReturnType *) realloc (returnkeylist,
						      sizeof (ReturnType) *
						      ((*num_return_keys) +
						       1));
			  ...
			  (returnkeylist + (*num_return_keys))->key2 =
			    tmp.key2;
			  strcpy ((returnkeylist +
				   (*num_return_keys))->description,
				  tmp.description);
...
//Convert a recieved string to a value
int
ConvertStreamtoInt (unsigned char *stream, int length, int *value)
{
  int i, nullvalue;
...
//Convert a recieved string to a value
time_t
ConvertStreamtoTime (unsigned char *stream, int length, time_t * value)
{
  int i, nullvalue;
...
void
SetSwitches (ConfType * conf, char *datefrom, char *dateto, int *location,
	     int *mysql, int *post, int *file, int *daterange, int *test)
{
  ...
  if ((strlen (conf->MySqlUser) > 0)
      ...
      && (strlen (conf->MySqlDatabase) > 0) && ((*test) == 0))
    (*mysql) = 1;
  else
    (*mysql) = 0;
  ...
  if ((strlen (conf->PVOutputURL) > 0)
      && (strlen (conf->PVOutputKey) > 0) && (strlen (conf->PVOutputSid) > 0))
    (*post) = 1;
  else
    (*post) = 0;
	    ...
	    int cc, int *terminated, int *togo)
{
  ...
  while (finished != 1)
    {
      datalist =
	(unsigned char *) realloc (datalist,
				   sizeof (char) * ((*datalen) +
						    (*streamlen) - i));
...
void
InitConfig (ConfType * conf, char *datefrom, char *dateto)
{
...
int
GetInverterSetting (ConfType * conf)
{
  FILE *fp;
  char line[400];
  char variable[400];
  ...
  if ((conf->InverterCode[0] == 0) ||
      (conf->InverterCode[1] == 0) ||
      (conf->InverterCode[2] == 0) ||
      (conf->InverterCode[3] == 0) || (conf->ArchiveCode == 0))
    {
...
PrintHelp ()
{
  ...
  printf ("\n");
  printf
    ("Dates are no longer required - defaults to last update if using mysql\n");
  ...
  printf
    ("Location Information to calculate sunset and sunrise so inverter is not\n");
  ...
  printf
    ("  -H,  --mysqlhost MYSQLHOST               mysql host default localhost\n");
  printf
    ("  -D,  --mysqldb MYSQLDATBASE              mysql database default smatool\n");
  printf ("  -U,  --mysqluser MYSQLUSER               mysql user\n");
  printf ("  -P,  --mysqlpwd MYSQLPASSWORD            mysql password\n");
  printf
    ("Mysql tables can be installed using INSTALL you may have to use a higher \n");
  printf
    ("privelege user to allow the creation of databases and tables, use command line \n");
  printf
    ("       --INSTALL                           install mysql data tables\n");
  printf
    ("       --UPDATE                            update mysql data tables\n");
...
/* Init Config to default values */
int
ReadCommandConfig (ConfType * conf, int argc, char **argv, char *datefrom,
		   char *dateto, int *verbose, int *debug, int *repost,
		   int *test, int *install, int *update)
{
  int i;
...
main (int argc, char **argv)
{
  ...
  int terminated = 0;
  int s, i, j, status, mysql = 0, post = 0, repost = 0, test = 0, file =
    0, daterange = 0;
  ...
  if (ReadCommandConfig
      (&conf, argc, argv, datefrom, dateto, &verbose, &debug, &repost, &test,
       &install, &update) < 0)
    exit (0);
  // read Config file
  if (GetConfig (&conf) < 0)
  ...
  SetSwitches (&conf, datefrom, dateto, &location, &mysql, &post, &file,
	       &daterange, &test);
  ...
  if ((daterange == 1) && ((location = 0) || (mysql == 0)))
	...
	start:
	  if (fgets (line, 400, fp) != NULL)
	    {			//read line from sma.in
	      linenum++;
	      lineread = strtok (line, " ;");
	      if (!strcmp (lineread, "R"))
		{		//See if line is something we need to receive
		  if (debug == 1)
		  ...
		  do
			...
			rr = 0;
			  ...
			  &&
			  (read_bluetooth
			   (&conf, &s, &rr, received, cc, last_sent,
			    &terminated) != 0))
			{
			  already_read = 0;
	      ...
	      if (!strcmp (lineread, "S"))
		{		//See if line is something we need to send
		  if (debug == 1)
		    printf ("[%d] %s Sending\n", linenum, debugdate ());
		  cc = 0;
		  do
			...
			case 2:	// $TIME    
			  ...
			  for (i = 7; i > 0; i = i - 2)
			    {	//change order and convert to integer
			      ti[1] = tt[i];
			      ti[0] = tt[i - 1];
			      ti[2] = '\0';
			      fl[cc] = conv (ti);
	      ...
	      if (!strcmp (lineread, "E"))
		{		//See if line is something we need to extract
		  if (debug == 1)
		  ...
		  do
			...
			case 3:	// Extract Serial of Inverter
			  ...
			  data =
			    ReadStream (&conf, &s, received, &rr, data,
			...
			case 5:	// extract current power $POW
			  ...
			  for (i = 0; i < datalen; i += gap)
			    {
			      idate =
				ConvertStreamtoTime (data + i + 4, 4, &idate);
			      loctime = localtime (&idate);
			      day = loctime->tm_mday;
			      month = loctime->tm_mon + 1;
			      year = loctime->tm_year + 1900;
			...
			case 6:	// extract total energy collected today
			  ...
			  gtotal =
			    (received[69] * 65536) + (received[68] * 256) +
			    received[67];
			  gtotal = gtotal / 1000;
			...
			case 12:	// extract time strings $TIMESTRING
			  if ((received[60] == 0x6d)
				...
				printf ("extracting timestring\n");
			      ...
			      idate =
				ConvertStreamtoTime (received + 63, 4,
						     &idate);
			  ...
			  while (finished != 1)
			      ...
			      for (i = 0; i < datalen; i++)
				{
				  ...
				  if (j > 11)
					...
					prev_idate = 0;
				      idate =
					ConvertStreamtoTime (datarecord, 4,
							     &idate);
				      if (prev_idate == 0)
					...
					idate;
				      ...
				      ConvertStreamtoLong (serial, 4,
							   ...
							   serial);
				      (archdatalist +
				       archdatalen)->accum_value =
			 gtotal / 1000;
				      (archdatalist +
				       archdatalen)->current_value =
			 (gtotal - ptotal) * 12;
				      archdatalen++;
				      ptotal = gtotal;
	      ...
	      if (!strcmp (lineread, ":init"))
		{		//See if line is something we need to extract
		  initstarted = 1;
		  returnpos = ftell (fp);
		  returnline = linenum;
		}
	      if (!strcmp (lineread, ":setup"))
		{		//See if line is something we need to extract
		  setupstarted = 1;
		  returnpos = ftell (fp);
		  returnline = linenum;
		}
	      if (!strcmp (lineread, ":startsetup"))
		{		//See if line is something we need to extract
		  sleep (1);
		}
	      if (!strcmp (lineread, ":setinverter1"))
		{		//See if line is something we need to extract
		  setupstarted = 1;
		  returnpos = ftell (fp);
		  returnline = linenum;
		}
	      if (!strcmp (lineread, ":getrangedata"))
		{		//See if line is something we need to extract
		  rangedatastarted = 1;
		  returnpos = ftell (fp);
		  returnline = linenum;
