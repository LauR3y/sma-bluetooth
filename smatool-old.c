                 ...
                 int *mysql, int *post, int *file, int *daterange, int *test)
{
  ...
  if ((strlen(conf->MySqlUser) > 0) && (strlen(conf->MySqlPwd) > 0) && (strlen(conf->MySqlHost) > 0) && (strlen(conf->MySqlDatabase) > 0) && ((*test) == 0))
    (*mysql) = 1;
  else
    (*mysql) = 0;
...
int main(int argc, char **argv)
{
  ...
  if (ReadCommandConfig(&conf, argc, argv, datefrom, dateto, &verbose, &debug, &repost, &test,
                        &install, &update) < 0)
    exit(0);
  // read Config file
  if (GetConfig(&conf) < 0)
        ...
        { //See if line is something we need to receive
          ...
          do
          {
            if (already_read == 0)
              rr = 0;
            if ((already_read == 0) &&
                (read_bluetooth(&conf, &s, &rr, received, cc, last_sent,
                                &terminated) != 0))
            {
              already_read = 0;
            ...
            else
            {
              ...
              if (memcmp(fl + 4, received + 4, cc - 4) == 0)
              {
                found = 1;
                if (debug == 1)
                  printf("[%d] %s Found string we are waiting for\n",
                         linenum, debugdate());
              }
              else
              {
          ...
          do
          {
            ...
            case 18: // $ARCHIVEDATA1
              ...
              while (finished != 1)
              {
                ...
                for (i = 0; i < datalen; i++)
                {
                  ...
                  if (j > 11)
                  {
                    ...
                    if (archdatalen == 0)
                      archdatalist =
                          (struct archdata_type *)
                              malloc(sizeof(struct archdata_type));
                    else
                      archdatalist =
                          (struct archdata_type *)
                ...
                if (togo == 0)
                  finished = 1;
                else if (read_bluetooth(&conf, &s, &rr, received, cc, last_sent,
                                        &terminated) != 0)
                {
                  fseek(fp, returnpos, 0);
                  linenum = returnline;
